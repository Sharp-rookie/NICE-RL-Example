...
def train():
    ...
    max_episode_timesteps = 1000        # max timesteps in one episode
    max_training_timesteps = int(3e6)   # break training loop if timeteps > max_training_timesteps

    # 打印日志、保存网络权重的频率
    print_freq = max_episode_timesteps * 10   # print avg reward in the interval (in num timesteps)
    log_freq = max_episode_timesteps * 2      # log avg reward in the interval (in num timesteps)
    save_model_freq = int(1e5)                # save model frequency (in num timesteps)

    # 对于连续动作，代码中取动作的分布为多元正态分布，actor输出的是正态分布的均值，而标准差则按如下参数进行衰减
    ...
    random_seed = 729       # set random seed if required (0 = no random seed)
    #####################################################
    ...
    if has_continuous_action_space:
        action_dim = env.action_space.shape[0]  # 连续动作的情况
    else:
        action_dim = env.action_space.n         # 离散动作的情况
    ...
    print("max timesteps per episode : ", max_episode_timesteps)
